...
// Type: RuntimeStyleSheet.UIElements.StyleSheets.CSSSpec
...
namespace RuntimeStyleSheet.UIElements.StyleSheets
{
  ...
  {
    private static readonly Regex rgx = new Regex("(?<id>#[-]?\\w[\\w-]*)|(?<class>\\.[\\w-]+)|(?<pseudoclass>:[\\w-]+(\\((?<param>.+)\\))?)|(?<type>[^\\-]\\w+)|(?<wildcard>\\*)|\\s+", RegexOptions.Compiled | RegexOptions.IgnoreCase);
